...
- Link: Inverse link is canonical for the Gamma distribution (P.S. log link is another good option as it ensures predicted values remain positive and are easier to interpret.)
```{R}
gamma_a <- glm(ahi ~ ageyears, data = subset(dmd_apnoea, ahi > 0), family = Gamma(link = "inverse"))
summary(gamma_a)
# Coefficients:
#               Estimate Std. Error t value Pr(>|t|)
# (Intercept)  0.1608796  0.0219398   7.333 6.82e-11 ***
# ageyears    -0.0015808  0.0005526  -2.861  0.00518 **
# ---
# Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1

# (Dispersion parameter for Gamma family taken to be 1.152831)

#     Null deviance: (127.8319) [171.7682] on 98  degrees of freedom
# Residual deviance: (120.3515) [163.7285] on 97  degrees of freedom
# AIC: (616.7018) [647.327] [0.04680549]
...
# Coefficients:
#               Estimate Std. Error t value Pr(>|t|)
# (Intercept)  0.1704868  0.0236259   7.216 9.75e-11 ***
# ageyears    -0.0017187  0.0005919  -2.904  0.00452 **
# ---
# Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1

# (Dispersion parameter for quasi family taken to be 1.264399)

#     Null deviance: [171.7682] on 103  degrees of freedom
# Residual deviance: [162.1849] on 102  degrees of freedom
# AIC: NA (616.1998) [643.6951] [0.05579233]
...
hurdle_gamma_a2 <- glm(ahi ~ ageyears, data = subset(dmd_apnoea, ahi > 0), family = Gamma(link = "inverse"))
...
(2) missing other relevant predictors.

```{R}
quasigamma_s <- glm(ahi ~ sex, data = dmd_apnoea, family = quasi(link = "inverse", variance = "mu^2"))
...
```{R}
quasigamma_as <- glm(ahi ~ ageyears + sex, data = dmd_apnoea, family = quasi(link = "inverse", variance = "mu^2"))
...
```{R}
quasigamma_t <- glm(ahi ~ type, data = dmd_apnoea, family = quasi(link = "inverse", variance = "mu^2"))
...
```{R}
quasigamma_at <- glm(ahi ~ ageyears + type, data = dmd_apnoea, family = quasi(link = "inverse", variance = "mu^2"))
summary(quasigamma_at)
gamma_gof(quasigamma_at, 2)   # 154.145  644.2782  644.2782  0.1025988
plot(quasigamma_at)
anova(quasigamma_at)
```

```{R}
quasigamma_ast <- glm(ahi ~ ageyears + type + sex, data = dmd_apnoea, family = quasi(link = "inverse", variance = "mu^2"))
summary(quasigamma_ast)
gamma_gof(quasigamma_ast, 2)   # 171.7682  153.5586  645.7525  0.1060131
plot(quasigamma_ast)
anova(quasigamma_ast, quasigamma_a)
```
...
```{R}
tweedie_21 <- tweedie_a
```

```{R}
anova(tweedie_15, tweedie_16, tweedie_2, tweedie_21)
```
